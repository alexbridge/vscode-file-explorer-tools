...
import { pendingRenames, releaseRenameGuard } from './rename-guard';
...
async function handleRename(
  ...
  // Guard against re-triggering from reverse cascade
  if (pendingRenames.has(newUri.fsPath)) {
    return;
  }
  pendingRenames.add(newUri.fsPath);
  try {
    ...
    releaseRenameGuard(newUri.fsPath, 500);
  }
}
